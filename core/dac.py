...
class DAC:
    ...
    def verify_proof(self, pp_dac, proof, D):
        ...
        flag_1 = self.setcommit.verify_cross(pp_sign, list_C, D, Witness_pi)
        flag_2 = self.zkp.verify(challenge, pedersen_open, pedersen_commit, nym_P, response)
        flag_3 = self.spseq_uc.verify(pp_sign,    vk_ca, nym_P, rndmz_commitment_vector,sigma_prime)
        return True
