"""
This implementation of SPSQE-VC signature. It uses proposed Vector commitments with Re-Random properties as an ingredient
...
class EQC_Sign:
    ...
    def rndmz_commit(self, commitment_vector, monypol_vector, mu):
        ...
        for i in range(len(monypol_vector)):
            rndmz_monypol_coeff = [mu * item for item in monypol_vector[i]]
            rndmz_monypol_vector.append(rndmz_monypol_coeff)
    ...
    def sign(self, pp_sign, param_acc, pk_u, sk, messages_vector, subset_indics, k_prime = None):
        ...
        for mess in messages_vector:
            ...
            ID_pk_u = Bn.from_decimal(str(random.randint(0, int(order) - 1)))
            # print(ID_pk_u)
            commit_2_vector.append(ID_pk_u * g_1 + pp_acc_G1[1])
            # print(len(commit_2_vector))
            ID_list.append(ID_pk_u)
            F_ID.append(ID_pk_u)
        # print(len(commit_2_vector))
        F_ID.append(1)
        # compute the unique identifier
        # ID_pk_u = order.random()
        # commit_2_vector = [ID_pk_u * g_1 + pp_acc_G1[1]]
        # ID_list = [ID_pk_u]
        # F_ID = [ID_pk_u,1]
        # print(len(commitment_vector))
        # pick randomness y
        y = order.random()
        ...
        # RA compute E1,E2
        E_1,E_2 = self.acc_scheme.BlindCom(param_acc, com_usign)

        # unblind com_usign
        usign_2_1 = {}
        for key,value in E_2.items():
            usign_2_1[key] = value + (t * E_1).neg()
        ...
        if k_prime != None:
            if k_prime > len(messages_vector):
                ...
                for item in range(len(messages_vector) + 1, k_prime + 1):
                    ...
                    usign_1[item] = UK
                    usign_2_1_k[item] = usign_2_1[item]
                    usign_2_2_k[item] = usign_2_2[item]
                update_key = {
                    'usign_1':usign_1,
                    'usign_2':[usign_2_1_k,usign_2_2_k]
                }
                return (sigma, update_key, commitment_vector, monypol_vector, commit_2_vector, F_ID, ID_list)
            else:
                print("not a good index, k_prime index should be greater  than message length")
        else:
            return (sigma, commitment_vector, monypol_vector,commit_2_vector, F_ID, ID_list)
    ...
    def Random_All(self, pp_sign, vk, pk_u, commitment_vector, monypol_vector, commit_2_vector, sigma, mu, upsilon, B=False, update_key=None):
        ...
        if B == True and update_key != None:
            ...
            for key in usign['usign_1']:
                ...
                usign_2_1_prime[key] = (mu * upsilon.mod_inverse(order)) * usign['usign_2'][0].get(key)
    ...
    def Delegate(self, pp_sign, param_acc, message_l, index_l, pk_l, sigma, commitment_vector, monypol_vector, commit_2_vector, F_ID, ID_list, update_key, subset_indics_L, mu=1, k_prime_double = None):
        """
        ...
        rndmz_monypol_L = [mu * monypol_L[i] for i in range(len(monypol_L))]

        
        # compute the unique identifier
        ID_pk_l = Bn.from_decimal(str(random.randint(0, int(order) - 1)))
        com_L_2 = ID_pk_l * g_1 + pp_acc_G1[1]
        rndmz_com_L_2 = mu * com_L_2
        ID_list.append(ID_pk_l)
        F_ID_prime = multiply_polynomial_by_binomial(F_ID,ID_pk_l,order)
        ...
        if (index_l in usign['usign_1']):
            ...
            usign_L_2 = [usign['usign_2'][0].get(index_l),usign['usign_2'][1].get(index_l)]
            points_uk_i = []
            for k,v in subset_indics_L.items():
                points_uk_i.append(set_l[k]* usign_L_1[v])
            points_uk = ec_sum(points_uk_i)
            V = points_uk + usign_L_2[0] + ID_pk_l * usign_L_2[1]
            Z_tilde = Z + V
            ...
            if k_prime_double is None:
                return (sigma_tilde, commitment_L, monypol_L, commitment_vector, monypol_vector, commit_2_vector, F_ID_prime,ID_list)
            else:
                usign_prime = {
                    'usign_1':[],
                    'usign_2':[]
                ...
                for i in range(index_l+1,k_prime_double+1):
                    ...
                    usign_prime["usign_2"].append([usign['usign_2'][0].get(i),usign['usign_2'][1].get(i)])
                    return (sigma_tilde, usign_prime, commitment_L, monypol_L, commitment_vector, monypol_vector, commit_2_vector, F_ID_prime,ID_list)
        else:
    ...
    def uk_verify(self, pp_sign, param_acc, vk, update_key, sigma):
        ...
        for key in usign['usign_1']:
            ...
            usign_2_1_list.append(usign['usign_2'][0].get(key))
        ...
        # usign_eq_2
        left_side_eq_2 = group.pair(pp_acc_G1[1],ec_sum(vk_x_prime_list))
        right_side_eq_2 = group.pair(ec_sum(usign_2_1_list),Y_hat)

        # usign_eq_3
        left_side_eq_3 = group.pair(g_1,ec_sum(vk_x_prime_list))
        right_side_eq_3 = group.pair(ec_sum(usign_2_2_list),Y_hat)

        return (left_side_eq_1 == right_side_eq_1) and (left_side_eq_2 == right_side_eq_2) and (left_side_eq_3 == right_side_eq_3)
